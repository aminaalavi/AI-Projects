...
DEFAULT_PASTED_MATTER = """matter_name: "Potential Antitrust Review for Tech Platform Expansion"
...
def extract_matter_from_text(matter_text: str, matter_name_hint: str) -> Dict[str, Any]:
    lines = [line.strip() for line in matter_text.splitlines() if line.strip()]
    parsed = {}
    ...
    if required_keys.issubset(parsed.keys()):
        summary_text = parsed.get("matter_summary", "")
        inferred_key_issues = []
        ...
        for term in keyword_bank:
            if term in summary_lower:
                inferred_key_issues.append(term)

        if not inferred_key_issues:
            inferred_key_issues = ["bundling", "market power", "regulatory scrutiny"]

        return {
            "matter_name": parsed.get("matter_name", matter_name_hint or "Uploaded Matter"),
            "practice_area": parsed.get("practice_area", "Antitrust"),
            "industry": parsed.get("industry", "Technology"),
            "client_type": parsed.get("client_type", "Public Company"),
            "matter_summary": summary_text,
            "key_issues": inferred_key_issues,
        }

    prompt = f"""
You are reading a matter description and converting it into a structured matter JSON.
...
"""
    out = llm_complete(
        "You extract structured legal matter metadata from matter text.",
        prompt
    )
    ...
    try:
        ...
        return {
            "matter_name": matter_name_hint or "Uploaded Matter",
            "practice_area": "Antitrust",
            "industry": "Technology",
            "client_type": "Public Company",
            "matter_summary": matter_text[:1000],
            "key_issues": ["bundling", "market power", "regulatory scrutiny"],
        }


st.set_page_config(page_title="Matter Risk Agents", page_icon="📊", layout="wide")
st.title("Matter Risk Agents")

st.info(
    "Paste or upload a matter description, set the matter name and number of rounds, "
    "and generate internal precedents, external context, a multi-agent risk review, "
    "and a final precedent stress-test memo."
)
...
if "new_matter" not in st.session_state:
    st.session_state.new_matter = None
...
with col_right:
    matter_name_input = st.text_input(
        "Matter Name",
        value="Potential Antitrust Review for Tech Platform Expansion"
    )
    rounds = st.slider("Number of Debate Rounds", min_value=1, max_value=4, value=2, step=1)

    matter_mode = st.radio(
        "How do you want to provide the matter input?",
        ["Upload .md/.txt file", "Paste matter text"],
        index=1,
    )
...
with col_left:
    ...
    if matter_mode == "Upload .md/.txt file":
        uploaded_file = st.file_uploader(
            "Upload matter file",
            type=["md", "txt"],
            help="Markdown or plain-text matter description.",
        )
    else:
        matter_text_input = st.text_area(
            "Paste matter text",
            value=DEFAULT_PASTED_MATTER,
            height=300,
            placeholder="Paste the full matter description here...",
        )

run_button = st.button("Run Matter Risk Agents")
    ...
    if not api_key:
        ...
        if matter_mode == "Upload .md/.txt file":
            if not uploaded_file:
                st.warning("Please upload a matter file or switch to paste mode.")
                matter_text = ""
            else:
                try:
                    matter_text = uploaded_file.read().decode("utf-8")
                except Exception as e:
                    st.error(f"Could not read uploaded file: {e}")
                    matter_text = ""
        else:
            matter_text = matter_text_input

        if matter_text and matter_text.strip():
            try:
                with st.spinner("Extracting matter and running precedent stress-test..."):
                    new_matter = extract_matter_from_text(matter_text, matter_name_input)
                ...
                st.session_state.new_matter = new_matter
...
if st.session_state.discussion or st.session_state.final_report:
    ...
    st.subheader("Multi-Agent Risk Review")
    ...
    if st.session_state.full_output:
        default_filename = (
            f"{(matter_name_input or 'Matter').replace(' ', '_')}"
            f"_PrecedentStressTest_{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.md"
        )
        st.download_button(
